...
class test_omsi_file_experiment(unittest.TestCase):
    ...
    def test_create_and_get_experiment(self):
        ...
        try:
            ...
            self.fail("Setting the experiment identifier failed. " + str(sys.exc_info()))
        self.assertEqual(exp.get_experiment_identifier()[:], exp_identifier_text)

        # Checking the number of experiments
        self.assertEqual(self.testfile.get_num_experiments(), 1)
