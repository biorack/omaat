...
class mzml_file(file_reader_base_multidata):
    ...
    def __compute_coordinates(self):
        ...
        if self.mzml_type == self.available_mzml_types['thermo']:
            ...
            for spectrum in reader:
                ...
                coords[spectrumid, :] = list(map(int, spotid.split(',')[-1].split('x')))
                spectrumid += 1
        ...
        for idx, spectrum in enumerate(reader):
            try:
                ...
                if scanfilter not in scantypes:
                    ...
                    scan_profiled.append('profile spectrum' in spectrum)
    ...
    def __parse_scan_parameters(self):
        ...
        for scan_idx, scantype in enumerate(self.scan_types):
            ...
            #MSnValueOfN
            n = [_f for _f in re.findall('(?<=ms)\d*', scantype) if _f]
            if n:
                MSnValueOfN = int(n[0])
            else:
                MSnValueOfN = 1
            ...
            #precursor
            ms2pre = [_f for _f in re.findall('[\d.]+(?=@)', scantype) if _f]
            if ms2pre:
                ms2_precursor = float(ms2pre[0])
            else:
                ms2_precursor = None
            #dissociation type
            dissot = [_f for _f in re.findall('(?<=\d@)[A-z]*', scantype) if _f]
            if dissot:
                dissociationtype = dissot[0]
            else:
                dissociationtype = 'None'
            #dissociation energy
            dissoe = [_f for _f in re.findall('(?<='+dissociationtype+')'+'[\d.]+', scantype) if _f]
            if dissoe:
                dissociationenergy = float(dissoe[0])
            else:
                dissociationenergy = None
            #polarity
            pol = [_f for _f in re.findall('([+][ ]p)', scantype) if _f]
            if pol:
                polarity = 'pos'
            else:
                pol = [_f for _f in re.findall('([-][ ]p)', scantype) if _f]
