...
def main(argv=None):
    ...
    if input_error:
        ...
        if ConvertSettings.job_id is not None:
            try:
                ...
                log_helper.warning(__name__, "Update of job-status to error failed " + str(e))
    ...
    if ConvertSettings.job_id is not None:
        try:
            ...
            log_helper.info(__name__, "Update of job-status to running failed " + str(e))
            ConvertSettings.recorded_warnings.append("Update of job-status to running failed " + str(e))
        except ValueError as e:
            log_helper.warning(__name__, "Update of job-status to running failed " + str(e))
            ConvertSettings.recorded_warnings.append("Update of job-status to running failed " + str(e))
    ...
    try:
        ...
        if ConvertSettings.job_id is not None:
            try:
                ...
                log_helper.warning(__name__, "Update of job-status to error failed " + str(e))
                ConvertSettings.recorded_warnings.append("Update of job-status to error failed " + str(e))
            except ValueError as e:
                log_helper.warning(__name__, "Update of job-status to error failed " + str(e))
                ConvertSettings.recorded_warnings.append("Update of job-status to error failed " + str(e))
        raise
    ...
    try:
        ...
        emailmsg = "Unexpected error creating the output file: " + str(sys.exc_info()[0])
        WebHelper.send_email(subject="ERROR: Conversion of file failed: " + omsi_outfile,
                             body=emailmsg,
                             email_type='error',
                             email_success_recipients=ConvertSettings.email_success_recipients,
    ...
    try:
        ...
        if ConvertSettings.error_handling == "terminate-and-cleanup":
            ...
            emailmsg += "  -Successfully deleted the generated HDF5 file: " + str(omsi_outfile) + " \n"
            log_helper.warning(__name__, "--Successfully deleted the generated HDF5 file: " + str(omsi_outfile))
        if ConvertSettings.error_handling == "terminate-only" or ConvertSettings.error_handling == "continue-on-error":
            emailmsg += "  -The generated HDF5 will not be added to the database." + " \n"
            log_helper.warning(__name__, "--The generated HDF5 will not be added to the database.")
            ConvertSettings.add_file_to_db = False
            emailmsg += "  -The output HDF5 file (if generate) remains at: " + str(omsi_outfile) + " \n"
            emailmsg += "  -Output file found: " + str(os.path.exists(omsi_outfile)) + " \n"
            log_helper.warning(__name__, "--The output HDF5 file (if generate) remains at: " + str(omsi_outfile))
            log_helper.warning(__name__, "  Output file found: " + str(os.path.exists(omsi_outfile)))
        emailmsg += "\n"
        emailmsg += str(sys.exc_info())
    ...
    if len(ConvertSettings.recorded_warnings) > 0:
        ...
        warningindex = 0
        for warn in ConvertSettings.recorded_warnings:
            warningmsg += "=========== " + str(warningindex) + " =========== \n"
            try:
                warningmsg += str(warn.message) + "\n"
    ...
    def parse_input_args(cls, argv):
        ...
        while i < (len(argv) - 1):
            ...
            if current_arg == "--no-nmf":
                ...
                if ConvertSettings.format_option not in list(ConvertSettings.available_formats.keys()):
                    log_helper.warning(__name__,
                                       "ERROR: Indicated --format option " +
                                       ConvertSettings.format_option +
                                       " not supported. Available options are:")
                    log_helper.info(__name__, "     " + str(list(ConvertSettings.available_formats.keys())))
                    input_error = True
                ...
                if error_option in ConvertSettings.available_error_options:
                    ...
                    current_arg.startswith("--notes"):
                start_index += 2
                ConvertSettings.metadata[current_arg] = str(argv[i+1])
...
class ConvertFiles(object):
            ...
            if has_method_metadata:
                ...
                if method_meta_key in ConvertSettings.metadata:
                    methods_info.add_metadata(metadata=metadata_value(
                        name='methods',
                        value=str(ConvertSettings.metadata[method_meta_key]),
                        description='Description of the experimental methods'))
                if notes_meta_key in ConvertSettings.metadata:
                    methods_info.add_metadata(metadata=metadata_value(
                        name='notes',
                        value=str(ConvertSettings.metadata[notes_meta_key]),
            ...
            if has_instrument_metadata:
                ...
                if instrument_meta_key in ConvertSettings.metadata:
                    instrument_info.add_metadata(metadata=metadata_value(
                                                 name='description',
                                                 value=str(ConvertSettings.metadata[instrument_meta_key]),
    ...
    def check_format(name, format_type):
        ...
        if format_type is not None:
            if format_type in list(ConvertSettings.available_formats.keys()):
                return format_type
            else:
                return None
        ...
        #    return "img"
        for formatname, formatclass in list(ConvertSettings.available_formats.items()):
            try:
            ...
            if supports_multidata:
                try:
                    tempfile = ConvertSettings.available_formats[currds['format']](
                        ...
                        requires_slicing=False)
                    for dataset_index in range(0, tempfile.get_number_of_datasets()):
                ...
                if data_region_option == 'split' or data_region_option == 'split+merge':
                    try:
                        tempfile = ConvertSettings.available_formats[currds['format']](
                            basename=currds['basename'],
                            requires_slicing=False)
                        for ri in range(0, tempfile.get_number_of_regions()):
                            nds = {'basename': currds['basename'],
                                   'format': currds['format'],
                                   'region': ri,
                                   'exp': 'previous'}
    ...
    def write_data(input_file, data, data_io_option="spectrum", chunk_shape=None, write_progress=True):
        ...
        if data_io_option == "spectrum" or (data_io_option == "chunk" and (chunk_shape is None)):
            ...
            if isinstance(input_file, file_reader_base.file_reader_base):
                ...
                for xindex in range(0, input_file.shape[0]):
                    if write_progress:
                        sys.stdout.write("[" + str(int(100. * float(xindex) / float(input_file.shape[0]))) + "%]" + "\r")
                        sys.stdout.flush()
                    for yindex in range(0, input_file.shape[1]):
            ...
            for zChunkIndex in range(0, num_chunks_z):
                ...
                for yChunkIndex in range(0, num_chunks_y):
                    ystart = yChunkIndex * chunk_shape[1]
                    yend = min(ystart + chunk_shape[1], ydim)
                    for xChunkIndex in range(0, num_chunks_x):
