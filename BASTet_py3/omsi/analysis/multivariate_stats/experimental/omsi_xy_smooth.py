...
###############################################################
class omsi_xy_smooth(analysis_base):
    """
    Class representing x,y smoothing of an image with a 2D-gaussian kernel.
    """

    def __init__(self, name_key="undefined"):
        """Initalize the basic data members"""

        super(omsi_xy_smooth, self).__init__()
        ...
        self.add_parameter(name='sigma_x',
                           help='The standard deviation in pixels for x-smoothing',
                           dtype=float,
                           required=True,
                           group=groups['input'])

        self.add_parameter(name='sigma_y',
                           help='The standard deviation in pixels for y-smoothing',
                           dtype=float,
                           required=True,
                           group=groups['input'])

        self.add_parameter(name='order',
                           help='Smoothing at order n will using a kernel nth derivative of a gaussian',
                           dtype=float,
                           required=False,
                           default=0,
                           group=groups['input'])
    ...
    def execute_analysis(self):
        ...
        new_msidata = gaussfilt(msidata, sigma=[sigma_x, sigma_y, 0], order=order, truncate=trunc)
        #return variables
    ...
    def v_qslice(cls,
        ...
        if viewer_option >= num_custom_viewer_options:
            return super(omsi_xy_smooth, cls).v_qslice(analysis_object,
                                                               z,
    ...
    def v_qspectrum(cls,
        ...
        if viewer_option >= num_custom_viewer_options:
            ...
            Replace omsi_xy_smooth with your classname
            """
            return super(omsi_xy_smooth, cls).v_qspectrum(analysis_object,
                                                                  x,
                                                                  y,
                                                                  viewer_option=viewer_option-num_custom_viewer_options)
    ...
    def v_qmz(cls, analysis_object, qslice_viewer_option=0, qspectrum_viewer_option=0):
        ...
        if qspectrum_viewer_option >= num_custom_spectrum_viewer_options \
            ...
            """EDIT_ME Replace the omsi_xy_smooth class name with your class name"""
            mz_spectra, label_spectra, mz_slice, label_slice, valuesX, labelX, valuesY, labelY, valuesZ, labelZ = \
                super(omsi_xy_smooth, cls)\
    ...
    def v_qspectrum_viewer_options(cls, analysis_object):
        ...
        Change the omsi_xy_smooth class-name to your class. If you did a
        replace all, then this should be done already.
        """
        dependent_options = super(omsi_xy_smooth, cls).v_qspectrum_viewer_options(analysis_object)
    ...
    def v_qslice_viewer_options(cls,
        ...
        """
        dependent_options = super(omsi_xy_smooth, cls).v_qslice_viewer_options(analysis_object)
