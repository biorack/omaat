    ...
    * ``v_qslice_viewer_options``: Define a list of strings, describing the different viewer options available for \
    ...
    def __getitem__(self,
        ...
        if isinstance(key, str) or isinstance(key, str):
            ...
            if len(key) == 2:
                if isinstance(key[0], str) or isinstance(key[0], str):
    ...
    def get_analysis_instances(cls):
        ...
        for ref in list(cls._analysis_instances.keys()):
        ...
        for ana_obj in cls.get_analysis_instances():
            ...
            for dataname in ana_params_and_outputs:
                ...
                if obj is data_object:
                    ...
                    # True in both cases.
                    if type(obj) not in (float, int, bool, int, complex, str, str):
                        return dependency_dict(param_name=None,
                                               link_name=None,
                                               dataname=dataname,
                                               omsi_object=ana_obj,
    ...
    def v_qmz(cls,
        ...
        if len(re_spectrum) > 0:
            if isinstance(re_spectrumdata[qspectrum_viewer_option], omsi_file_msidata):
                ...
                label_spectra = "m/z"
                valuesX = list(range(0, re_spectrumdata[qspectrum_viewer_option].shape[0]))
                valuesY = list(range(0, re_spectrumdata[qspectrum_viewer_option].shape[1]))
                is3Dimage = len(re_spectrumdata[qspectrum_viewer_option].shape)
                valuesZ = None if is3Dimage <= 3 else list(range(0, re_spectrumdata[qspectrum_viewer_option].shape[2]))
    ...
    def clear_and_restore(self, analysis_manager=None, resave=False):
        ...
        print(list(self.get_omsi_analysis_storage()[0].managed_group.items()))
    ...
    def set_parameter_values(self,
        ...
        """
        log_helper.debug(__name__, "Setting analysis parameters. " + str(list(kwargs.keys())),
                         root=self.mpi_root, comm=self.mpi_comm)
        import h5py
        ...
        for k, v in list(kwargs.items()):
            ...
            dtype = curr_parameter['dtype'] if curr_parameter is not None else str  # unicode
